function [v,size]=waist(lambda,beam_waist,wp,fpos,f,show_flag)
...
if (nargin < 4); fpos=[0 0.25]; end
if (nargin < 3);  wp=-0.25; end
if (nargin < 2); beam_waist=1e-3; end
...
% text anotations:
text(fpos(2:end),0.95*yline(1,:),strcat('f=',num2str(f')));    % prints the focal length of each lens
text(fpos(2:end),-0.95*yline(1,:),strcat('Z_f=',num2str(fpos(2:end)')));    % prints the focal length of each lens

if show_flag
% plots the waist location and size at different heights
ymax=max(yline(:));
text(waist_pos,w-0.05*ymax,strcat('W_{',num2str((1:length(waist_pos))'),'}=',num2str(w','%0.2d')),'FontSize',8);
